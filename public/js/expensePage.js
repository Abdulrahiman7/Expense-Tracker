...
async function expenseDownload(e)
{
    e.preventDefault();
    try{
        const x=await axios.get('http://localhost:4000/premium/download',{headers})
        console.log(x);
        if(x.status===200)
        {
...
function displayPreviousDownloads(download)
{
    ...
    const a=document.createElement('a');
    a.textContent=`${download.createdAt}`;
    a.href=`${download.URL}`;
    li.appendChild(a);
    ul.appendChild(li);
}
...
{
    try{
        const x=await axios.get('http://localhost:4000/premium/expenseReport',{headers});
        if(x.status===200)
        {
